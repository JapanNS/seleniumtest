...
public class AmazonSteps {
    ...
    public void userShouldSeeAddedToCart() {
        ...
        Assert.assertTrue("Add to Cart Message is not displayed", addToCartMessage.isDisplayed());
        assertEquals("Added to Cart message is invalid", "Added to Cart", addToCartMessage.getText());
    }
    ...
    public void userLoginShouldBeResult(String result) {
        if(!result.equalsIgnoreCase("fail")) {
            ...
            assertEquals("wrong User name displayed in the sign in message", expected, signInMessage.getText());
        }else {
            WebElement importantMessage = wait.
                    until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[contains(text(),'Important Message!')]")));
            assertEquals("Important Message!", importantMessage.getText());
        }
    }
